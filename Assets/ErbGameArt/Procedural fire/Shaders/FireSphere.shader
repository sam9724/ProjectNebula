Shader "EGA/Particles/FireSphere"
{
	...
	SubShader
	{
		...
		Pass
		{
			...
			TEXTURE2D(_tex3coord); SAMPLER(sampler_tex3coord);
			...
			half4 frag(Varyings input) : SV_Target
			{
				...
				float4 temp_output_121_0 = (_Emission * _Color * input.vertexColor);

				float2 temp_output_8_0 = (((float2(0.2, 0) * _Time.y) + (input.uv_tex3coord).xy + input.uv_tex3coord.z) * _StartFrequency);
				...
				float3 temp_output_51_0 = ((float3((float2(0.5, 0.5) * _Time.y), 0.0) + (input.uv_tex3coord * (lerpResult40 * _Amplitude)) + input.uv_tex3coord.z) * _Frequency);
				...
				float4 tex2DNode117 = SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, (input.uv_tex3coord + (0.2 * (lerpResult106 * Amp114))).xy);

				float4 clampResult132 = clamp((input.vertexColor.a * tex2DNode117 * _Opacity), float4(0, 0, 0, 0), float4(1, 1, 1, 1));
				float4 ase_screenPos = float4(input.screenPos.xyz, (input.screenPos.w + 0.00000000001));
				float4 ase_screenPosNorm = ase_screenPos / ase_screenPos.w;

				ase_screenPosNorm.z = (UNITY_NEAR_CLIP_VALUE >= 0) ? ase_screenPosNorm.z : ase_screenPosNorm.z * 0.5 + 0.5;

				float screenDepth137 = LinearEyeDepth(LOAD_TEXTURE2D_LOD(_CameraDepthTexture, ase_screenPosNorm.xy, 0), float4(1, 1, 1, 1));
				float distanceDepth137 = abs((screenDepth137 - LinearEyeDepth(ase_screenPosNorm.z, float4(1, 1, 1, 1))) / (_Depthpower));
				float clampResult136 = clamp(distanceDepth137, 0.0, 1.0);

				//emission + alpha
				half3 emission = lerp(temp_output_121_0, (temp_output_121_0 * tex2DNode117), _Useblack).rgb;
				half alpha = lerp(clampResult132, (clampResult132 * clampResult136), _Usedepth).r;
