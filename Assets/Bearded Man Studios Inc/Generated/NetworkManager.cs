...
namespace BeardedManStudios.Forge.Networking.Unity
{
	public partial class NetworkManager : MonoBehaviour
	{
		...
		public GameObject[] SeekerNetworkObject = null;
		...
		private void CaptureObjects(NetworkObject obj)
		{
			...
			else if (obj is DroneNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (DroneNetworkObject.Length > 0 && DroneNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(DroneNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<DroneBehavior>();
						}
					}
			...
			else if (obj is GunnerNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (GunnerNetworkObject.Length > 0 && GunnerNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(GunnerNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<GunnerBehavior>();
						}
					}
			...
			else if (obj is MothershipNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (MothershipNetworkObject.Length > 0 && MothershipNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(MothershipNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<MothershipBehavior>();
						}
					}
			...
			else if (obj is PilotNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					...
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (PilotNetworkObject.Length > 0 && PilotNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(PilotNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<PilotBehavior>();
						}
					}
			...
			else if (obj is Player1PositionNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (Player1PositionNetworkObject.Length > 0 && Player1PositionNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(Player1PositionNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<Player1PositionBehavior>();
						}
					}
			...
			else if (obj is SeekerNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (SeekerNetworkObject.Length > 0 && SeekerNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(SeekerNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<SeekerBehavior>();
						}
					}
		...
		[Obsolete("Use InstantiateDrone instead, its shorter and easier to type out ;)")]
		public DroneBehavior InstantiateDroneNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(DroneNetworkObject[index]);
			var netBehavior = go.GetComponent<DroneBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<DroneBehavior>().networkObject = (DroneNetworkObject)obj;
		...
		[Obsolete("Use InstantiateGunner instead, its shorter and easier to type out ;)")]
		public GunnerBehavior InstantiateGunnerNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(GunnerNetworkObject[index]);
			var netBehavior = go.GetComponent<GunnerBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<GunnerBehavior>().networkObject = (GunnerNetworkObject)obj;

			FinalizeInitialization(go, netBehavior, obj, position, rotation, sendTransform);
			
			return netBehavior;
		}
		[Obsolete("Use InstantiateMothership instead, its shorter and easier to type out ;)")]
		public MothershipBehavior InstantiateMothershipNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(MothershipNetworkObject[index]);
			var netBehavior = go.GetComponent<MothershipBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<MothershipBehavior>().networkObject = (MothershipNetworkObject)obj;
		...
		{
			var go = Instantiate(PilotNetworkObject[index]);
			var netBehavior = go.GetComponent<PilotBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<PilotBehavior>().networkObject = (PilotNetworkObject)obj;

			FinalizeInitialization(go, netBehavior, obj, position, rotation, sendTransform);
			
			return netBehavior;
		}
		[Obsolete("Use InstantiatePlayer1Position instead, its shorter and easier to type out ;)")]
		public Player1PositionBehavior InstantiatePlayer1PositionNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(Player1PositionNetworkObject[index]);
			var netBehavior = go.GetComponent<Player1PositionBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<Player1PositionBehavior>().networkObject = (Player1PositionNetworkObject)obj;

			FinalizeInitialization(go, netBehavior, obj, position, rotation, sendTransform);
			
			return netBehavior;
		}
		[Obsolete("Use InstantiateSeeker instead, its shorter and easier to type out ;)")]
		public SeekerBehavior InstantiateSeekerNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(SeekerNetworkObject[index]);
			var netBehavior = go.GetComponent<SeekerBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<SeekerBehavior>().networkObject = (SeekerNetworkObject)obj;
		...
		/// <summary>
		/// Instantiate an instance of Drone
		/// </summary>
		/// <returns>
		/// A local instance of DroneBehavior
		/// </returns>
		/// <param name="index">The index of the Drone prefab in the NetworkManager to Instantiate</param>
		/// <param name="position">Optional parameter which defines the position of the created GameObject</param>
		/// <param name="rotation">Optional parameter which defines the rotation of the created GameObject</param>
		/// <param name="sendTransform">Optional Parameter to send transform data to other connected clients on Instantiation</param>
		public DroneBehavior InstantiateDrone(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(DroneNetworkObject[index]);
			var netBehavior = go.GetComponent<DroneBehavior>();
		...
		/// <summary>
		/// Instantiate an instance of Gunner
		/// </summary>
		/// <returns>
		/// A local instance of GunnerBehavior
		/// </returns>
		/// <param name="index">The index of the Gunner prefab in the NetworkManager to Instantiate</param>
		/// <param name="position">Optional parameter which defines the position of the created GameObject</param>
		/// <param name="rotation">Optional parameter which defines the rotation of the created GameObject</param>
		/// <param name="sendTransform">Optional Parameter to send transform data to other connected clients on Instantiation</param>
		public GunnerBehavior InstantiateGunner(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(GunnerNetworkObject[index]);
			var netBehavior = go.GetComponent<GunnerBehavior>();
		...
		/// <summary>
		/// Instantiate an instance of Mothership
		/// </summary>
		/// <returns>
		/// A local instance of MothershipBehavior
		/// </returns>
		/// <param name="index">The index of the Mothership prefab in the NetworkManager to Instantiate</param>
		/// <param name="position">Optional parameter which defines the position of the created GameObject</param>
		/// <param name="rotation">Optional parameter which defines the rotation of the created GameObject</param>
		/// <param name="sendTransform">Optional Parameter to send transform data to other connected clients on Instantiation</param>
		public MothershipBehavior InstantiateMothership(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(MothershipNetworkObject[index]);
			var netBehavior = go.GetComponent<MothershipBehavior>();
		...
		/// </summary>
		/// <returns>
		/// A local instance of PilotBehavior
		/// </returns>
		/// <param name="index">The index of the Pilot prefab in the NetworkManager to Instantiate</param>
		/// <param name="position">Optional parameter which defines the position of the created GameObject</param>
		/// <param name="rotation">Optional parameter which defines the rotation of the created GameObject</param>
		/// <param name="sendTransform">Optional Parameter to send transform data to other connected clients on Instantiation</param>
		public PilotBehavior InstantiatePilot(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(PilotNetworkObject[index]);
			var netBehavior = go.GetComponent<PilotBehavior>();
		...
		/// <summary>
		/// Instantiate an instance of Player1Position
		/// </summary>
		/// <returns>
		/// A local instance of Player1PositionBehavior
		/// </returns>
		/// <param name="index">The index of the Player1Position prefab in the NetworkManager to Instantiate</param>
		/// <param name="position">Optional parameter which defines the position of the created GameObject</param>
		/// <param name="rotation">Optional parameter which defines the rotation of the created GameObject</param>
		/// <param name="sendTransform">Optional Parameter to send transform data to other connected clients on Instantiation</param>
		public Player1PositionBehavior InstantiatePlayer1Position(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(Player1PositionNetworkObject[index]);
			var netBehavior = go.GetComponent<Player1PositionBehavior>();
		...
		/// <summary>
		/// Instantiate an instance of Seeker
		/// </summary>
		/// <returns>
		/// A local instance of SeekerBehavior
		/// </returns>
		/// <param name="index">The index of the Seeker prefab in the NetworkManager to Instantiate</param>
		/// <param name="position">Optional parameter which defines the position of the created GameObject</param>
		/// <param name="rotation">Optional parameter which defines the rotation of the created GameObject</param>
		/// <param name="sendTransform">Optional Parameter to send transform data to other connected clients on Instantiation</param>
		public SeekerBehavior InstantiateSeeker(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(SeekerNetworkObject[index]);
			var netBehavior = go.GetComponent<SeekerBehavior>();
